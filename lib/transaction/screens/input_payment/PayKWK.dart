...
class PayKwk extends StatelessWidget {
  ...
  PayKwk({
    required this.nohpResult,
    required this.name,
    required this.tipe,
    required this.noHpUser,
    required this.pinUser,
    required this.angkotName,
    required this.angkotImage,
  });
...
class PayKwkPageState extends State<PayKwkPage> {
  ...
  @override
  Widget build(BuildContext context) {
    Uint8List bytes = base64.decode(widget.angkotImage);
  ...
  void kirim() async {
    ...
    try {
      print('OEEEE');
      var url = Uri.parse('https://bill.co.id/resultTransac');
